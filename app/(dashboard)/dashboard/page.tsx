            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: '24px' }}>
                ...
                <div>
                    <div style={{ fontSize: '10px', fontFamily: 'Courier New, monospace', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#4b5563', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ width: '2px', height: '12px', background: '#65a30d', display: 'inline-block', borderRadius: '1px' }} />
                        Projects
                    </div>
                        ...
                        {projects.map(p => (
                            <div key={p.id} style={{ background: '#ffffff', border: '1px solid rgba(0,0,0,0.07)', borderRadius: '12px', padding: '20px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '24px' }}>
                                ...
                                <Link
                                    ...
                                    style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '8px 16px', background: 'rgba(163,230,53,0.08)', border: '1px solid rgba(163,230,53,0.2)', borderRadius: '8px', color: '#65a30d', fontSize: '11px', fontWeight: 700, fontFamily: 'Courier New, monospace', letterSpacing: '0.08em', textTransform: 'uppercase', whiteSpace: 'nowrap', textDecoration: 'none', transition: 'background 0.15s' }}
                    ...
                    <div style={{ background: '#ffffff', border: '1px solid rgba(0,0,0,0.07)', borderRadius: '12px', overflow: 'hidden' }}>
                        {chats.map(c => (
                            <Link
                                ...
                                style={{ display: 'block', padding: '16px 20px', borderBottom: '1px solid rgba(0,0,0,0.05)', textDecoration: 'none', transition: 'background 0.15s' }}
                                onMouseEnter={e => (e.currentTarget.style.background = 'rgba(0,0,0,0.02)')}
                                onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
                            >
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                                    <span style={{ fontSize: '11px', fontWeight: 700, fontFamily: 'Courier New, monospace', color: '#65a30d', letterSpacing: '0.06em', textTransform: 'uppercase' }}>{c.user}</span>
                                    <span style={{ fontSize: '10px', color: '#4b5563' }}>{c.time}</span>
                                </div>
                                <p style={{ margin: 0, fontSize: '13px', color: '#6b7280', lineHeight: 1.5, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
                                    &quot;{c.message}&quot;
                                </p>
                            </Link>
                        ))}
