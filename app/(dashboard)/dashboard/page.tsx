    ...
    return (
        <div className="p-6 md:p-12 min-h-full">
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-16 relative">
                ...
                <div>
                    <h1 className={`text-6xl font-black uppercase italic tracking-tighter ${comicMode ? 'comic-font drop-shadow-[6px_6px_0px_rgba(230,36,41,0.2)]' : japaneseMode ? 'japanese-font not-italic text-[#BE1E2D] tracking-[0.1em] drop-shadow-sm' : neonMode ? 'neon-font text-[#FAF0E6] tracking-tight drop-shadow-[0_0_20px_rgba(245,166,35,0.3)]' : hashiMode ? 'hashi-font text-[#1a1a1a] tracking-[0.1em] font-normal drop-shadow-[0_0_30px_rgba(0,0,0,0.05)]' : 'tracking-tight'}`}>
                        {hashiMode ? 'DASHBOARD' : comicMode ? 'Command Center' : 'Dashboard Overview'}
                    </h1>
                    <p className={`mt-2 font-bold uppercase text-xs ${japaneseMode ? 'japanese-font text-[#1A2639] tracking-[0.3em] not-italic' : neonMode ? 'neon-font text-[#F5A623]/60 tracking-widest' : hashiMode ? 'hashi-font text-[#c20000] tracking-[0.4em] font-normal border-l-2 border-[#c20000] pl-3' : 'text-zinc-600 tracking-widest'}`}>The world awaits, {session?.user?.name || 'Traveler'}</p>
                </div>
                <div className={`${comicMode
                    ...
                    : japaneseMode
                        ...
                        : neonMode
                            ...
                            : hashiMode
                                ? 'hashi-font cyber-border text-[#1a1a1a] bg-[#c20000]/5 tracking-[0.2em] font-bold uppercase text-xs shadow-[0_0_30px_rgba(0,0,0,0.02)] px-8 backdrop-blur-xl status-pulse'
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-20">
                ...
                <div className={`p-1 transition-all ${comicMode ? 'bg-white border-4 border-black shadow-[12px_12px_0px_#000] hover:scale-105' : japaneseMode ? 'bg-transparent border border-[#E0D8C8] hover:border-[#BE1E2D]' : neonMode ? 'bg-[#111118] border border-[#F5A623]/15 hover:border-[#F5A623]/40 shadow-[0_0_24px_rgba(245,166,35,0.04)]' : hashiMode ? 'card overflow-hidden h-full flex flex-col justify-center' : 'bg-white border border-zinc-200 rounded-2xl shadow-sm hover:shadow-md'}`}>
                    <div className={`${comicMode ? 'bg-hero-blue p-8' : japaneseMode ? 'bg-[#FAF8F5]/80 backdrop-blur-sm p-8 relative overflow-hidden' : neonMode || hashiMode ? 'p-8 relative overflow-hidden' : 'p-8'} text-black`}>
                        ...
                        {hashiMode && <div className="absolute -right-2 -bottom-3 opacity-[0.02] font-black text-9xl text-[#c20000] hashi-font pointer-events-none">I</div>}
                        <h3 className={`text-[10px] font-black uppercase tracking-[0.2em] mb-4 ${comicMode ? 'comic-font text-white/60' : japaneseMode ? 'japanese-font text-[#1A2639] tracking-[0.3em] not-italic' : neonMode ? 'neon-font text-[#F5A623]/50' : hashiMode ? 'hashi-font text-[#1a1a1a]/60 tracking-[0.3em] font-medium' : 'text-zinc-400'}`}>{hashiMode ? 'HOME BASE' : 'Active Operations'}</h3>
                ...
                {/* Card 2 */}
                <div className={`p-1 transition-all ${comicMode ? 'bg-white border-4 border-black shadow-[12px_12px_0px_#000] hover:scale-105' : japaneseMode ? 'bg-transparent border border-[#E0D8C8] hover:border-[#BE1E2D]' : neonMode ? 'bg-[#111118] border border-[#E8547A]/15 hover:border-[#E8547A]/40 shadow-[0_0_24px_rgba(232,84,122,0.04)]' : hashiMode ? 'card overflow-hidden h-full flex flex-col justify-center' : 'bg-white border border-zinc-200 rounded-2xl shadow-sm hover:shadow-md'}`}>
                    <div className={`${comicMode ? 'bg-marvel-red p-8' : japaneseMode ? 'bg-[#FAF8F5]/80 backdrop-blur-sm p-8 relative overflow-hidden' : neonMode || hashiMode ? 'p-8 relative overflow-hidden' : 'p-8'} text-black`}>
                        ...
                        {hashiMode && <div className="absolute -right-2 -bottom-3 opacity-[0.02] font-black text-9xl text-[#9a0000] hashi-font pointer-events-none">II</div>}
                        <h3 className={`text-[10px] font-black uppercase tracking-[0.2em] mb-4 ${comicMode ? 'comic-font text-white/60' : japaneseMode ? 'japanese-font text-[#1A2639] tracking-[0.3em] not-italic' : neonMode ? 'neon-font text-[#E8547A]/50' : hashiMode ? 'hashi-font text-[#1a1a1a]/60 font-medium tracking-[0.3em]' : 'text-zinc-400'}`}>{hashiMode ? 'ALERTS' : 'Strategic Alerts'}</h3>
                ...
                {/* Card 3 */}
                <div className={`p-1 transition-all ${comicMode ? 'bg-white border-4 border-black shadow-[12px_12px_0px_#000] hover:scale-105' : japaneseMode ? 'bg-transparent border border-[#E0D8C8] hover:border-[#1A2639]' : neonMode ? 'bg-[#111118] border border-[#F5A623]/10 hover:border-[#F5A623]/30' : hashiMode ? 'card overflow-hidden h-full flex flex-col justify-center border-[#c20000]/10 hover:border-[#c20000]/30' : 'bg-white border border-zinc-200 rounded-2xl shadow-sm hover:shadow-md'}`}>
                    <div className={`${comicMode ? 'bg-comic-yellow p-8' : japaneseMode ? 'bg-[#FAF8F5]/80 backdrop-blur-sm p-8 relative overflow-hidden' : neonMode || hashiMode ? 'p-8 relative overflow-hidden' : 'p-8'} text-black`}>
                        ...
                        {hashiMode && <div className="absolute -right-2 -bottom-3 opacity-[0.02] font-black text-9xl text-[#c20000] pointer-events-none">III</div>}
                        <h3 className={`text-[10px] font-black uppercase tracking-[0.2em] mb-4 ${comicMode ? 'text-black/50' : japaneseMode ? 'text-[#1A2639] font-serif tracking-[0.3em] not-italic' : neonMode ? 'text-[#F5A623]/50' : hashiMode ? 'text-[#1a1a1a]/60 font-normal tracking-[0.3em] hashi-font' : 'text-zinc-400'}`}>{hashiMode ? 'VAULT SEC' : 'Vault Security'}</h3>
                    ...
                    <div className={`${comicMode
                        ...
                        : japaneseMode
                            ...
                            : neonMode
                                ...
                                : hashiMode
                                    ? 'text-xs text-[#c20000] uppercase tracking-[0.4em] mb-4 border-l-2 border-[#c20000] pl-3 font-bold'
                                    : 'text-xl font-bold font-black uppercase italic'
                        } mb-8 transition-all`}>
                        {hashiMode ? 'PROJECTS' : 'High-Stakes Missions'}
                    </div>
                    <div className="grid grid-cols-1 gap-8">
                        {[
                            { id: 'bar-man', title: 'The Bar-man', color: 'marvel-red', icon: Zap, description: 'Premium mixology platform for clandestine networking events.' },
                            { id: 'space-balls', title: 'Space-balls', color: 'hero-blue', icon: Target, description: 'Sub-orbital logistics and high-speed asset recovery systems.' }
                        ].map(proj => (
                            <div key={proj.id} className={`${comicMode ? 'bg-white p-1 border-4 border-black shadow-[8px_8px_0px_#000] hover:scale-[1.01]' : japaneseMode ? 'bg-transparent border border-[#E0D8C8] hover:border-[#1A2639]' : neonMode ? 'bg-[#111118] border border-[#F5A623]/10 hover:border-[#F5A623]/30 hover:shadow-[0_0_20px_rgba(245,166,35,0.06)]' : hashiMode ? 'card' : 'bg-white border border-zinc-100 rounded-2xl shadow-sm hover:border-zinc-300'} transition-all`}>
                                <div className={`${comicMode ? 'bg-white/50 p-8 flex justify-between items-center border-2 border-dashed border-zinc-200' : japaneseMode ? 'bg-[#FAF8F5]/50 p-8 flex flex-col md:flex-row justify-between items-start md:items-center' : neonMode || hashiMode ? 'p-6 flex justify-between items-center' : 'p-8 flex justify-between items-center'} gap-6`}>
                                    <div>
                                        <div className={`text-4xl uppercase tracking-tighter transition-colors ${comicMode ? `font-black italic group-hover:text-${proj.color} text-black` : japaneseMode ? 'font-serif tracking-[0.1em] text-[#1A2639]' : neonMode ? 'font-black text-[#FAF0E6]' : hashiMode ? 'hashi-font font-bold text-[#1a1a1a] tracking-[0.1em]' : 'font-black italic text-black'}`}>{proj.title}</div>
                                        <div className={`text-xs uppercase mt-2 italic flex items-center gap-2 ${japaneseMode ? 'text-[#1A2639]/70 font-serif tracking-[0.2em] not-italic' : neonMode ? 'text-[#FAF0E6]/30 not-italic tracking-widest' : hashiMode ? 'hashi-font text-[#c20000] not-italic tracking-[0.15em] font-medium' : 'text-zinc-500 font-bold tracking-widest'}`}>
                                            <proj.icon size={14} className={comicMode ? (proj.id === 'bar-man' ? 'animate-pulse text-marvel-red' : `text-${proj.color}`) : neonMode ? 'text-[#F5A623]/50' : japaneseMode ? 'text-[#BE1E2D]' : hashiMode ? 'text-[#c20000]' : ''} /> {hashiMode ? proj.description : (proj as any).status}
                                        </div>
                                    </div>
                                    <Link href="/missions">
                                        <Button className={comicMode
                                            ...
                                            : japaneseMode
                                                ...
                                                : neonMode
                                                    ...
                                                    : hashiMode
                                                        ? 'hashi-font cyber-border bg-[#c20000]/5 text-[#c20000] hover:bg-[#c20000]/10 hover:text-[#1a1a1a] rounded-none px-8 font-bold tracking-[0.15em] uppercase text-[10px] shadow-[inset_0_0_10px_rgba(255,255,255,0.5)]'
                                                        : "rounded-full px-6"}>
                                            {hashiMode ? 'JUMP IN' : 'ENTER INTEL'}
                                        </Button>
                                    </Link>
                                </div>
                            </div>
                    ...
                    <div className={`${comicMode
                        ...
                        : japaneseMode
                            ...
                            : neonMode
                                ...
                                : hashiMode
                                    ? 'text-xs text-[#c20000]/80 uppercase tracking-[0.4em] mb-4 border-l-2 border-[#c20000] pl-3 font-bold'
                                    : 'text-xl font-bold font-black uppercase italic'
                        } mb-8 transition-all`}>
                        {hashiMode ? 'DIRECT INQUIRIES' : 'Live Comms'}
                    </div>
                        ...
                        {mockChats.map((chat) => (
                            <div key={chat.id} className={`${comicMode ? 'p-5 bg-zinc-50 border-l-8 border-comic-yellow transform hover:scale-[1.02] hover:-rotate-1 border-y-2 border-r-2 border-black shadow-sm' : japaneseMode ? 'p-4 border-l border-l-[#BE1E2D] bg-[#FDFBF7]' : neonMode ? 'p-4 border-l-2 border-l-[#E8547A]/40 bg-[#E8547A]/[0.02] hover:border-l-[#E8547A]/60' : hashiMode ? 'p-4 border-l-2 border-l-[#c20000]/60 bg-[#c20000]/5 hover:border-l-[#c20000]/90 transition-all shadow-[inset_0_0_15px_rgba(255,255,255,0.5)]' : 'p-4 bg-zinc-50 rounded-xl'} transition-all`}>
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <span className={`text-[9px] ${japaneseMode ? 'text-[#1A2639]/50 font-serif' : neonMode ? 'text-[#FAF0E6]/20' : hashiMode ? 'hashi-font text-[#1a1a1a]/40' : 'text-zinc-400 font-black italic'}`}>{chat.time}</span>
                                </div>
                                <p className={`text-lg leading-tight ${comicMode ? 'text-black font-bold italic' : japaneseMode ? 'text-[#1A2639] font-serif not-italic' : neonMode ? 'text-[#FAF0E6]/70' : hashiMode ? 'hashi-font text-[#1a1a1a] font-normal' : 'text-zinc-800 font-bold italic'}`}>"{chat.message}"</p>
                            </div>
                        ))}
                        <Link href="/comms" className="w-full">
                            <Button className={`${comicMode ? 'comic-font bg-black text-white border-4 border-black rounded-none h-14 hover:bg-marvel-red text-xl shadow-[4px_4px_0px_rgba(230,36,41,0.5)] font-black italic' : japaneseMode ? 'w-full japanese-font bg-transparent text-[#1A2639] border border-[#1A2639] rounded-none hover:bg-[#1A2639] hover:text-white tracking-[0.2em]' : hashiMode ? 'w-full cyber-border hashi-font bg-[#c20000]/5 text-[#c20000] hover:bg-[#c20000]/10 hover:text-[#1a1a1a] rounded-none tracking-[0.2em] font-bold h-12' : 'w-full rounded-xl font-black italic'} uppercase flex items-center justify-center gap-2 transition-all`}>
